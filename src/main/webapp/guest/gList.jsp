...
<head>
  <meta charset="UTF-8">
  <title>gList.jsp(방명록리스트_블록페이징처리(1,2,3))</title>
...
<% if(pag != 1) {  %>
		<a href="gList.jsp?pag=1" class="btn btn-secondary btn-sm"> 첫 페이지</a>	
<% } %>
<% if(curBlock > 0) {  %>
		<a href="gList.jsp?pag=<%=(curBlock-1)*blockSize + 1%>" class="btn btn-secondary btn-sm"> 이전블록</a>	
<% } %>
<%
	...
	for(int i=(curBlock*blockSize)+1 ; i<=(curBlock*blockSize)+blockSize; i++){
		if(i>totPage) break;
		if(i==pag) out.println("<a href='gList.jsp?pag="+i+"'><font color='purple'>"+i+"</font></a>"); 
		else out.println("<a href='gList.jsp?pag="+i+"'>"+i+"</a>");
	}
%>
<% if(curBlock < lastBlock) {  %>
		<a href="gList.jsp?pag=<%=(curBlock+1)*blockSize + 1%>" class="btn btn-secondary btn-sm"> 다음블록</a>	
<% } %>
<% if(pag != totPage) {  %>
		<a href="gList.jsp?pag=<%=totPage%>" class="btn btn-secondary btn-sm"> 마지막 페이지</a>	
<% } %>
